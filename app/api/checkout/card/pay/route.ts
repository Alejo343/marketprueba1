...
function generateReference() {
  ...
  return `BARRIL-CD-${Date.now()}-${rand}`;
}
...
export async function POST(req: NextRequest) {
  const { cardToken, amountInCents, customerEmail, installments } = await req.json();

  if (!cardToken || !amountInCents || !customerEmail) {
  ...
  if (!txRes.ok) {
    ...
    console.error("Wompi Card error:", errData);
    const msg =
      errData?.error?.messages?.payment_method?.[0] ??
      errData?.error?.reason ??
      errData?.error?.message ??
      "Error al crear el pago con tarjeta";
