...
function CheckoutItem({
  ...
  return (
    <div className="flex gap-4 py-5 border-b border-(--color-divider) last:border-0">
      <div
        className="shrink-0 rounded-xl overflow-hidden bg-(--color-subtle-bg) border border-(--color-border) flex items-center justify-center"
      ...
      <div className="flex-1 min-w-0">
        <p className="text-(--color-text) text-sm font-medium leading-snug line-clamp-2">
          {item.name}
        </p>
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex items-center bg-(--color-subtle-bg) border border-(--color-border) rounded-lg overflow-hidden">
            <button
              ...
              className="w-8 h-8 flex items-center justify-center text-(--color-muted) hover:text-(--color-primary) hover:bg-(--color-primary)/10 transition-all duration-150 cursor-pointer text-base font-light"
            >
              −
            </button>
            <span className="w-8 text-center text-(--color-text) text-sm font-semibold">
              {item.quantity}
            </span>
            <button
              onClick={() => updateQuantity(item.variantId, item.quantity + 1)}
          ...
          <div className="flex items-center gap-3">
            <span
              className="text-(--color-primary) font-bold text-sm"
            ...
            <button
              ...
              className="text-(--color-muted)/50 hover:text-red-400 transition-colors duration-150 cursor-pointer p-1"
              title="Eliminar"
            >
              <TrashIcon />
            </button>
...
function inputCls(hasError: boolean) {
  return [
    ...
    hasError
      ...
      : "border-(--color-input-border) focus:border-(--color-primary)/50",
  ].join(" ");
}
...
export default function CheckoutPage() {
  ...
  const [nequiStep, setNequiStep] = useState<
    "idle" | "waiting" | "approved" | "declined"
  >("idle");
  const [nequiError, setNequiError] = useState("");
  const [nequiTxRef, setNequiTxRef] = useState<{
    id: string;
    reference: string;
  } | null>(null);
  ...
  // PSE state
  const [pseInstitutions, setPseInstitutions] = useState<
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/checkout/pse/institutions`)
      .then((r) => r.json())
      .then((d) => {
        if (Array.isArray(d.institutions)) setPseInstitutions(d.institutions);
      })
      .catch(() => {
        /* ignore */
      });
  ...
  const [deliveryCostCents, setDeliveryCostCents] = useState<number | null>(
    null,
  );
  const [deliveryVariantId, setDeliveryVariantId] = useState<number | null>(null);
  ...
  const detectDeliveryZone = async (coords?: { lat: number; lng: number }) => {
    ...
    try {
      ...
      setDeliveryError(
        err instanceof Error ? err.message : "Error calculando envío",
      );
  ...
  const validate = () => {
    ...
    if (!form.identification.trim())
      errs.identification = `${form.identificationType} requerido`;
    else if (form.identification.replace(/\D/g, "").length < 5)
      errs.identification = "Mín. 5 dígitos";
    if (form.identificationType === "NIT" && !form.businessName.trim())
      errs.businessName = "Razón social requerida";
    if (!form.city.trim()) errs.city = "Ciudad requerida";
    if (!form.address.trim()) errs.address = "Dirección requerida";
    setErrors(errs);
    if (!mapCoords) {
      setMapError(
        "Debes confirmar tu ubicación en el mapa para calcular el envío",
      );
      return false;
    }
    setMapError("");
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/checkout/nequi/pay`, {
        ...
        body: JSON.stringify({
          ...
          deliveryVariantId: deliveryVariantId ?? null,
    ...
    const poll = setInterval(async () => {
      try {
        const res = await fetch(
          `${apiUrl}/checkout/orders/${nequiTxRef.reference}/status`,
        );
        ...
        if (status === "APPROVED") {
          ...
          setNequiError(
            "El pago fue rechazado por Nequi. Verifica tu saldo e intenta de nuevo.",
          );
        }
      } catch {
  ...
  const handleCardPay = async () => {
    ...
    try {
      ...
      if (!tokRes.ok || tokData.status !== "CREATED") {
        throw new Error(
          tokData?.error?.messages?.number?.[0] ??
            tokData?.error?.reason ??
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      try {
        const res = await fetch(
          `${apiUrl}/checkout/orders/${cardTxRef.reference}/status`,
        );
  ...
  return (
    <>
      <Header />
      <div
        className="min-h-screen bg-(--color-bg) pt-8 pb-20"
        style={{ fontFamily: "var(--font-inter)" }}
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <Link
            href="/"
            className="inline-flex items-center gap-1.5 text-(--color-muted) hover:text-(--color-primary) text-sm mb-8 transition-colors duration-200 cursor-pointer"
          >
          ...
          {/* Title */}
          <div className="mb-10">
            <p className="text-(--color-primary) text-xs font-semibold tracking-widest uppercase mb-1">
              Finalizar compra
            </p>
            <h1
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 items-start">
              ...
              <div className="space-y-6">
                ...
                {/* Datos de entrega */}
                <div className="bg-(--color-surface) rounded-2xl border border-(--color-primary)/10 p-6">
                  <h2 className="text-(--color-text) font-semibold text-base mb-5">
                    Datos de entrega
                  </h2>
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <Field label="Teléfono / WhatsApp *" error={errors.phone}>
                        <div className="relative flex items-center">
                          <span className="absolute left-4 text-(--color-muted) text-sm pointer-events-none select-none">
                            +57
                          </span>
                          <input
                            type="tel"
                        ...
                        <div className="flex gap-2">
                          <select
                            ...
                            onChange={(e) =>
                              setForm((prev) => ({
                                ...prev,
                                identificationType: e.target.value as DocType,
                                businessName: "",
                              }))
                            }
                          ...
                          <input
                            ...
                            className={
                              inputCls(!!errors.identification) +
                              " flex-1 min-w-0"
                            }
                          />
                        </div>
                    ...
                    <div>
                      ...
                      {mapError && (
                        <p className="text-red-400 text-xs mt-1">{mapError}</p>
                      )}
                    </div>
                ...
                <div className="bg-(--color-surface) rounded-2xl border border-(--color-primary)/10 p-6">
                    ...
                    !deliveryError && (
                      <div className="bg-amber-900/20 border border-amber-500/30 rounded-xl px-4 py-3 mb-5">
                        <p className="text-amber-300 text-xs leading-relaxed">
                          Una vez procesado tu pedido, te enviaremos por
                          WhatsApp el valor del envío a tu zona.
                        </p>
                      </div>
                    )}
                  ...
                  {cardStep === "waiting" ? (
                    <div className="text-center py-6 space-y-4">
                      ...
                      <div>
                        <p className="text-(--color-text) font-semibold text-sm">
                          Procesando tu tarjeta
                        </p>
                        <p className="text-(--color-muted) text-xs mt-1">
                          Autorizando cobro de{" "}
                          <span className="text-(--color-text)">
                            {fmt(total)}
                          </span>
                        </p>
                      ...
                      <p className="text-[#9A8C7A] text-[11px]">
                    ...
                    <div className="text-center py-6 space-y-4">
                      <div
                        ...
                        style={{
                          background:
                            "linear-gradient(135deg, #DA0081 0%, #B5006C 100%)",
                        }}
                      >
                        <svg
                          width="28"
                        ...
                        >
                          <path
                            d="M10 29V11l13 13.5V11h6v18L16 15.5V29H10Z"
                            fill="white"
                          />
                        </svg>
                      </div>
                      <div>
                        <p className="text-(--color-text) font-semibold text-sm">
                          Aprueba el pago en Nequi
                        </p>
                        <p className="text-(--color-muted) text-xs mt-1">
                          Abre tu app y acepta la solicitud de{" "}
                          <span className="text-(--color-text)">
                            {fmt(total)}
                          </span>
                        </p>
                    ...
                    <>
                      ...
                      <div className="mb-5">
                        <p className="text-(--color-muted) text-[11px] font-semibold uppercase tracking-wider mb-2.5">
                          Método de pago
                        </p>
                      ...
                      <div className="mb-4 space-y-2.5 bg-(--color-subtle-bg) border border-(--color-border) rounded-xl p-3.5">
                        <label className="flex items-start gap-2.5 cursor-pointer group">
                          ...
                          <span className="text-(--color-muted) text-[11px] leading-snug">
                            ...
                            {terms ? (
                              ...
                              <span className="text-(--color-primary)/60">
                                Términos y Condiciones de Uso
                              </span>
                            )}{" "}
                            de Wompi.
                        ...
                        {termsError && (
                          <p className="text-red-400 text-[11px] pt-0.5">
                            {termsError}
                          </p>
                        )}
                      </div>
                      ...
                      {payMethod === "nequi" && (
                        <div className="space-y-3">
                          ...
                          <button
                            type="button"
                            onClick={handleNequiPay}
                            disabled={nequiLoading}
                        ...
                        <div className="space-y-3">
                          <Field label="Banco" error={pseErrors.bank}>
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                setPseErrors((p) => ({ ...p, bank: "" }));
                              }}
                              className={
                                inputCls(!!pseErrors.bank) + " cursor-pointer"
                              }
                              autoFocus
                            >
                              <option value="">
                                {pseInstitutions.length === 0
                                  ? "Cargando bancos..."
                                  : "Selecciona tu banco"}
                              </option>
                          ...
                          <Field label="Tipo de persona">
                            <select
                              value={pseUserType}
                              onChange={(e) =>
                                setPseUserType(e.target.value as "0" | "1")
                              }
                              className={inputCls(false) + " cursor-pointer"}
                            >
                          ...
                          <button
                            ...
                            style={{
                              background:
                                "linear-gradient(135deg, #0057A6 0%, #003C73 100%)",
                            }}
                          >
                            {pseLoading ? (
                              <>
                  ...
                  <div className="flex items-center justify-center gap-2 mt-3">
                    <span className="text-(--color-muted) text-[11px]">
                      Procesado por
                    </span>
                    <WompiLogo />
                  </div>
                ...
                <div className="bg-(--color-surface) rounded-2xl border border-(--color-primary)/10 p-5">
                  <div className="space-y-3">
                    {[
                      ...
                      <div
                        key={text}
                        className="flex items-center gap-3 text-xs text-(--color-muted)"
                      >
