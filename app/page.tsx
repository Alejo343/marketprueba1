...
const trustItems = [
  { icon: <TruckIcon />, title: "Envío gratis", desc: "En pedidos mayores a $80.000" },
...
const productCategories = [
  "Abarrotes e Ingredientes", "Aceites", "Arroces especiales", "Asadores y Parrilla",
  "Carnes y Embutidos", "Carnicería", "Condimentos", "Congelados", "Conservas",
  "Especias", "Harinas", "Licores y vinos", "Pastas", "Productos importados",
  "Salsas", "Snacks", "Utensilios", "Utensilios de Cocina",
];

const featuredProducts = [
  { id: 1, name: "Carbón Vegetal Premium 2kg", price: 38900, original: 45900, image: "/images/product-thumb-1.png", rating: 5, reviews: 128, badge: "Más vendido" },
  { id: 2, name: "Set Pinzas y Espátula Pro", price: 89900, original: null, image: "/images/product-thumb-11.jpg", rating: 4, reviews: 67, badge: null },
  { id: 3, name: "Delantal Parrillero Negro", price: 54900, original: 64900, image: "/images/product-thumb-12.jpg", rating: 5, reviews: 203, badge: "20% off" },
  { id: 4, name: "Salsa Gourmet Artesanal", price: 24900, original: null, image: "/images/product-thumb-13.jpg", rating: 4, reviews: 89, badge: "Nuevo" },
  { id: 5, name: "Ahumador Tipo Barril 40L", price: 329000, original: 380000, image: "/images/product-thumb-14.jpg", rating: 5, reviews: 45, badge: "Premium" },
  { id: 6, name: "Miel de Colmena 500g", price: 32900, original: null, image: "/images/product-thumb-2.png", rating: 5, reviews: 156, badge: null },
  { id: 7, name: "Kit Especias Mexicanas", price: 44900, original: 52900, image: "/images/product-thumb-1.png", rating: 4, reviews: 72, badge: "15% off" },
  { id: 8, name: "Pasta Artesanal Italiana", price: 18900, original: null, image: "/images/product-thumb-11.jpg", rating: 5, reviews: 311, badge: null },
];
...
function ProductCard({ product }: { product: typeof featuredProducts[0] }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div className="p-4">
        <p className="text-[#F5F0E8] text-sm font-medium leading-snug mb-2 line-clamp-2">{product.name}</p>
        <div className="flex items-center gap-0.5 mb-3">
          {Array.from({ length: 5 }).map((_, i) => (
            <span key={i} className={i < product.rating ? "text-[#C9A84C]" : "text-[#333]"}>
              <StarIcon filled={i < product.rating} />
            </span>
          ))}
          <span className="text-[#9A8C7A] text-xs ml-1.5">({product.reviews})</span>
        </div>
        <div className="flex items-baseline gap-2">
          ...
          {product.original && (
            <span className="text-[#9A8C7A] text-xs line-through">{formatPrice(product.original)}</span>
          )}
        </div>
      </div>
    </div>
...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/product-variants").then((r) => r.json()).then((res) => setLatestVariants((res.data ?? []).slice(0, 8))).catch(() => {});
  }, []);
    ...
    <div className="min-h-screen bg-[#080808]" style={{ fontFamily: "var(--font-inter)" }}>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-4 h-[72px]">
              <Link href="/" className="flex-shrink-0">
                <img src="/logov2w.webp" alt="Barril Market" className="h-14 w-auto" />
              </Link>
              <div className="hidden lg:flex flex-1 mx-8">
                <div className="w-full flex items-center bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg px-4 py-2.5 gap-3 focus-within:border-[#C9A84C]/60 transition-all duration-200">
                  <span className="text-[#C9A84C]/60"><SearchIcon /></span>
              ...
              <div className="ml-auto flex items-center gap-3">
                ...
                <button className="flex items-center gap-2 bg-[#C9A84C] hover:bg-[#B8973D] text-[#111111] px-5 py-2.5 rounded-lg font-semibold text-sm transition-all duration-200 cursor-pointer">
                  <CartIcon /><span className="hidden sm:inline">Carrito</span>
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full py-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-[3.5rem] lg:text-[4.5rem] font-bold text-[#F5F0E8] leading-[1.1] mb-6 whitespace-pre-line"
                style={{ fontFamily: "var(--font-playfair)" }}>
                {current.title}
              </h1>
              <p className="text-[#9A8C7A] text-lg leading-relaxed mb-8 max-w-md">{current.subtitle}</p>
              ...
              <div className="flex items-center gap-4 flex-wrap">
                <Link href="/region/accesorios-y-vitrinas"
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {trustItems.map((item, i) => (
              <div key={i} className="flex items-center gap-3 px-4">
                ...
                <div>
                  <p className="text-[#F5F0E8] text-sm font-semibold">{item.title}</p>
            ...
            <div className="lg:col-span-7 bg-[#111111] rounded-2xl border border-[#C9A84C]/15 overflow-hidden relative">
              ...
              <div className="flex flex-col md:flex-row h-full p-8 gap-6 items-center">
                <div className="flex-1">
                  <span className="text-[#C9A84C] text-xs font-semibold tracking-widest uppercase block mb-3">BBQ Profesional</span>
                  <h2 className="text-[2.2rem] font-bold text-[#F5F0E8] leading-tight mb-3"
                    style={{ fontFamily: "var(--font-playfair)" }}>
                    Asador & Ahumador<br />Tipo Barril
                  </h2>
                  <p className="text-[#9A8C7A] text-sm mb-6 leading-relaxed">Controla la temperatura y logra el sabor ahumado perfecto.</p>
                  <Link href="/region/accesorios-y-vitrinas"
                ...
                <div className="flex-shrink-0 flex items-center justify-center md:w-48">
                  <img src="/images/barril.png" alt="Asador" className="max-h-[240px] object-contain drop-shadow-xl" />
                </div>
              </div>
            </div>
            <div className="lg:col-span-5 flex flex-col gap-4">
              {[
                { badge: "20% Off", title: "Carbón Premium para Parrilla", image: "/images/ad-image-1.png", href: "/region/accesorios-y-vitrinas" },
                { badge: "15% Off", title: "Delantales para Parrilleros", image: "/images/ad-image-2-2.png", href: "/region/accesorios-y-vitrinas" },
              ].map((b) => (
                <Link key={b.badge} href={b.href}
                  ...
                  <div className="absolute inset-0 bg-no-repeat bg-right-bottom opacity-35 group-hover:opacity-50 transition-opacity duration-300"
                  ...
                  <div className="relative z-10 p-7 flex flex-col justify-between h-full">
                    ...
                    <div>
                      <h3 className="text-[1.25rem] font-bold text-[#F5F0E8] mb-2"
                        style={{ fontFamily: "var(--font-playfair)" }}>{b.title}</h3>
      ...
      <section className="py-12 bg-[#080808]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <span className="text-[#C9A84C] text-xs font-semibold tracking-widest uppercase block mb-2">Nuestras tiendas</span>
              <h2 className="text-[2rem] font-bold text-[#F5F0E8]" style={{ fontFamily: "var(--font-playfair)" }}>
                Explora por región
              </h2>
            </div>
            <Link href="/" className="hidden sm:flex items-center gap-2 text-[#C9A84C] hover:text-[#D4BC6A] text-sm font-medium transition-colors duration-200 cursor-pointer">
              Ver todas <ArrowIcon />
            </Link>
          </div>
            ...
            {regions.map((cat) => (
              <Link key={cat.href} href={cat.href}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <p className="text-[#F5F0E8] text-xs font-semibold leading-tight">{cat.label}</p>
      ...
      <section className="py-12 bg-[#080808]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <span className="text-[#C9A84C] text-xs font-semibold tracking-widest uppercase block mb-2">Selección especial</span>
              <h2 className="text-[2rem] font-bold text-[#F5F0E8]" style={{ fontFamily: "var(--font-playfair)" }}>
                Productos destacados
              </h2>
            </div>
      ...
      <section className="py-12 bg-[#080808]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <span className="text-[#C9A84C] text-xs font-semibold tracking-widest uppercase block mb-2">Recién llegados</span>
              <h2 className="text-[2rem] font-bold text-[#F5F0E8]" style={{ fontFamily: "var(--font-playfair)" }}>
                Últimos productos
              </h2>
            </div>
          ...
          {latestVariants.length === 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="bg-[#111111] rounded-2xl overflow-hidden border border-[#C9A84C]/10 animate-pulse">
                  ...
                  >
                    <div className="relative aspect-square bg-[#0D0D0D] overflow-hidden">
                      {img ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-[#C9A84C]/20 text-5xl font-bold" style={{ fontFamily: "var(--font-playfair)" }}>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-baseline gap-2">
                        <span className="text-[#C9A84C] font-bold text-base" style={{ fontFamily: "var(--font-playfair)" }}>
                          {new Intl.NumberFormat("es-CO", { style: "currency", currency: "COP", minimumFractionDigits: 0 }).format(variant.final_price)}
                        </span>
                        {variant.has_sale && variant.price !== variant.final_price && (
                          <span className="text-[#9A8C7A] text-xs line-through">
                            {new Intl.NumberFormat("es-CO", { style: "currency", currency: "COP", minimumFractionDigits: 0 }).format(variant.price)}
                          </span>
                        )}
                      </div>
                    </div>
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div className="relative max-w-2xl mx-auto text-center px-4">
          ...
          <h2 className="text-[2.5rem] font-bold text-[#F5F0E8] mb-4" style={{ fontFamily: "var(--font-playfair)" }}>
            Productos artesanales,<br />experiencias únicas
          </h2>
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            ...
            <span className="text-[#9A8C7A] text-sm">o llámanos al <span className="text-[#C9A84C]">+57 300 000 0000</span></span>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-[#0A0A0A] border-t border-[#C9A84C]/10 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between flex-wrap gap-4">
          ...
          <p className="text-[#9A8C7A] text-xs">© 2025 Barril Market · Todos los derechos reservados</p>
          <div className="flex gap-6">
            {["Términos", "Privacidad", "Contacto"].map((l) => (
              <span key={l} className="text-[#9A8C7A] hover:text-[#C9A84C] text-xs transition-colors duration-200 cursor-pointer">{l}</span>
            ))}
          </div>
        </div>
      </footer>
